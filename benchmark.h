/* JMM: Taken from Daniel Lemires Blog
 * https://raw.githubusercontent.com/lemire/Code-used-on-Daniel-Lemire-s-blog/master/2018/01/09/benchmark.h
*/
...
#define BEST_TIME_NOCHECK(test, pre, repeat, verbose, id)                         \
        do {                                                              \
            if (global_rdtsc_overhead == UINT64_MAX) {                    \
               RDTSC_SET_OVERHEAD(1, repeat);        \
            }                                                             \
            ...
            if(verbose) printf(" %zu %s per function (best) ", min_diff, unitname);   \
            if(verbose) printf("\n");                                                 \
            if(!verbose) printf("%zu %d \n",min_diff, id);                   \
            fflush(NULL);                                                 \
 } while (0)
