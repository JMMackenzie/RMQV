...
namespace ds2i {
    ...
    struct score_opt_partition {
                            ...
                            std::vector<float>& doc_weights)
        {
            ...
            fill_partition_weights(doc_weights);
        }

        // Given the document weights, computes max weight for each v-block
        void fill_partition_weights(std::vector<float>& doc_weights){

          size_t lower_bound = 0;
          for (size_t i = 0; i < partition.size(); ++i) {
              size_t upper_bound = partition[i];
              // Find max in bounds
              float max_weight = *std::max_element(doc_weights.begin() + lower_bound, doc_weights.begin() +upper_bound);
